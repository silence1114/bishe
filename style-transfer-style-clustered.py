...
dir_path = '/home/silence/test/photos_test/'
...
def style_trans(input_im,ref_im):
    ...
    #面部校正结果
    fig = plt.figure()
    without_face_corr = color.lab2rgb(output_im_luminance)
    ax = fig.add_subplot(1,2,1)
    ax.imshow(without_face_corr)
    ax.set_title('Before face correction')
    plt.axis('off')
    ax = fig.add_subplot(1,2,2)
    ax.imshow(output_image)
    ax.set_title('After face correction')
    plt.axis('off')
    plt.show()
    '''
...
def luminance_transfer(input_im,ref_im,output_im):
    ...
    tmp_luminance_feature = input_luminance_feature + (ref_luminance_feature-input_luminance_feature)*(1/2)
...
if __name__ == '__main__':
    ...
    # 载入风格类排序结果
    cluster_ranking_file = open(save_path+'style-cluster-ranking.pkl', 'rb')
    cluster_ranking_all = pickle.load(cluster_ranking_file)
    # 载入风格聚类结果
    ...
    for photo in sorted(photo_list):
        ...
        cluster_ranking = cluster_ranking_all[label].copy()
        top_clusters = cluster_ranking.argsort()[::-1][:top_k] #[x:y:z]切片索引,x是左端,y是右端,z是步长,在[x,y)区间从左到右每隔z取值,默认z为1可以省略z参数.步长的负号就是反向,从右到左取值.
        j = 2
        for top_cluster in  top_clusters:

            indexs = ref_cluster_index[top_cluster]
            top_image = style_ranking[indexs].argsort()[::-1][0]
            ref_im = io.imread(ref_path+ref_imagenames[indexs[top_image]])
